...
#define NS_DECL_NON_VIRTUAL_INISEARCHPROCESS \
  NS_METHOD GetIsActive(bool *aIsActive); \
  NS_METHOD GetResultCount(int32_t *aResultCount); \
  NS_METHOD GetHoldResults(bool *aHoldResults); \
  NS_METHOD SetHoldResults(bool aHoldResults); \
  NS_METHOD SearchSync(void); \
  NS_METHOD SearchAsync(inISearchObserver *aObserver); \
  NS_METHOD SearchStop(void); \
  NS_METHOD SearchStep(bool *_retval); \
  NS_METHOD GetStringResultAt(int32_t aIndex, nsAString & _retval); \
  NS_METHOD GetIntResultAt(int32_t aIndex, int32_t *_retval); \
  NS_METHOD GetUIntResultAt(int32_t aIndex, uint32_t *_retval); 
