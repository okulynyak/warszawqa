...
#define NS_DECL_NON_VIRTUAL_IMGIENCODER \
  NS_METHOD InitFromData(const uint8_t *data, uint32_t length, uint32_t width, uint32_t height, uint32_t stride, uint32_t inputFormat, const nsAString & outputOptions); \
  NS_METHOD StartImageEncode(uint32_t width, uint32_t height, uint32_t inputFormat, const nsAString & outputOptions); \
  NS_METHOD AddImageFrame(const uint8_t *data, uint32_t length, uint32_t width, uint32_t height, uint32_t stride, uint32_t frameFormat, const nsAString & frameOptions); \
  NS_METHOD EndImageEncode(void); \
  NS_METHOD GetImageBufferUsed(uint32_t *_retval); \
  NS_METHOD GetImageBuffer(char **_retval); 
...
/* Implementation file */
NS_IMPL_ISUPPORTS(_MYCLASS_, imgIEncoder)
